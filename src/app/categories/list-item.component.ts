...
export class CategoriesListItemComponent {
  ...
  @Input() set parentRouterLink(parentRouterLink: string[]) { this.parentRouterLink$.next(parentRouterLink); };
  public parentRouterLink$ = new BehaviorSubject<string[]>(null);

  public isModer$ = this.acl.isAllowed(Resource.GLOBAL, Privilege.MODERATE);
  public havePhoto$ = this.item$.pipe(
    map(item => item ? this.isHavePhoto(item) : false)
  );
  public canHavePhoto$ = this.item$.pipe(
    ...
    ].indexOf(item.item_type_id) !== -1 : false
  ));

  public pictures$: Observable<PictureThumbRoute[]> = combineLatest([this.item$, this.parentRouterLink$]).pipe(
    map(([item, parentRouterLink]) => item.preview_pictures.pictures.map(pic => ({
      picture: pic ? pic.picture : null,
      thumb: pic ? pic.thumb : null,
      route: pic && pic.picture && parentRouterLink ? parentRouterLink.concat(['pictures', pic.picture.identity]) : null
    })))
  );
